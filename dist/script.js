...
Elm.Grid.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var nextRow = F2(function (row,accumulatorGrid) {    return accumulatorGrid;});
   var getNeighboringIndices = F2(function (_p1,_p0) {
      var _p2 = _p1;
      var _p7 = _p2._1;
      var _p6 = _p2._0;
      var _p3 = _p0;
      ...
      function (_p4) {
         var _p5 = _p4;
         return A2($List.member,_p5._0,_U.range(0,_p3._0)) && A2($List.member,_p5._1,_U.range(0,_p3._1));
      },
      A2($List.filter,
      function (item) {
         return !_U.eq(item,{ctor: "_Tuple2",_0: _p6,_1: _p7});
      },
      A3($List$Extra.lift2,F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),_U.range(_p6 - 1,_p6 + 1),_U.range(_p7 - 1,_p7 + 1))));
   });
   var cellAt = F3(function (x,y,grid) {
      var row = A2($Maybe.withDefault,_U.list([]),$List.head(A2($List.drop,y,grid)));
      return A2($Maybe.withDefault,false,$List.head(A2($List.drop,x,row)));
   });
   var sumWhenTrue = F2(function (status,sum) {    return status ? sum + 1 : sum;});
   var getLiveNeighbourCount = F3(function (x,y,grid) {
      var maxY = $List.length(grid);
      var maxX = $List.length(A2($Maybe.withDefault,_U.list([]),$List.head(grid)));
      var neighbouring_indices = A2(getNeighboringIndices,{ctor: "_Tuple2",_0: x,_1: y},{ctor: "_Tuple2",_0: maxX,_1: maxY});
      var neighbours = A2($List.map,function (_p8) {    var _p9 = _p8;return A3(cellAt,_p9._0,_p9._1,grid);},neighbouring_indices);
      return A3($List.foldl,sumWhenTrue,0,neighbours);
   });
   var transformCell = F4(function (grid,y,x,state) {
      var liveNeighbours = A3(getLiveNeighbourCount,x,y,grid);
      return _U.eq(liveNeighbours,3) || _U.eq(liveNeighbours,2) && state;
   });
   var transformRow = F3(function (grid,y,row) {    return A2($List.indexedMap,A2(transformCell,grid,y),row);});
   var nextGrid = function (grid) {    return A2($List.indexedMap,transformRow(grid),grid);};
   var drawCell = F2(function (cellSize,cellStatus) {
      return A2($Graphics$Element.color,cellStatus ? $Color.black : $Color.blue,A2($Graphics$Element.spacer,cellSize,cellSize));
   });
   var appendCell = F3(function (cellSize,cellStatus,element) {
      return A2($Graphics$Element.flow,$Graphics$Element.right,_U.list([element,A2(drawCell,cellSize,cellStatus)]));
   });
   var drawRow = F2(function (cellSize,list) {    return A3($List.foldl,appendCell(cellSize),A2($Graphics$Element.spacer,0,0),list);});
   var appendRow = F3(function (cellSize,row,element) {
      return A2($Graphics$Element.flow,$Graphics$Element.down,_U.list([element,A2(drawRow,cellSize,row)]));
   });
   var drawGrid = F2(function (cellSize,grid) {    return A3($List.foldl,appendRow(cellSize),A2($Graphics$Element.spacer,0,0),grid);});
   return _elm.Grid.values = {_op: _op,drawGrid: drawGrid,nextGrid: nextGrid};
};
Elm.SeedGenerator = Elm.SeedGenerator || {};
Elm.SeedGenerator.make = function (_elm) {
   "use strict";
   _elm.SeedGenerator = _elm.SeedGenerator || {};
   if (_elm.SeedGenerator.values) return _elm.SeedGenerator.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var padGrid = F2(function (x,row) {    var paddingValues = A2($List.repeat,x - $List.length(row),false);return A2($List.append,row,paddingValues);});
   var useTemplate = F3(function (templateGrid,maxX,maxY) {
      var xPaddedGrid = A2($List.map,padGrid(maxX),templateGrid);
      var emptyRow = A2($List.repeat,maxX,false);
      var paddingRows = A2($List.repeat,maxY - $List.length(templateGrid),emptyRow);
      return A2($List.append,xPaddedGrid,paddingRows);
   });
   var spaceship = _U.list([_U.list([false,false,false,false,false])
                           ,_U.list([false,false,true,false,false])
                           ,_U.list([false,false,false,true,false])
                           ,_U.list([false,true,true,true,false])
                           ,_U.list([false,false,false,false,false])]);
   var oscillator = _U.list([_U.list([false,false,false,false,false])
                            ,_U.list([false,false,true,false,false])
                            ,_U.list([false,false,true,false,false])
                            ,_U.list([false,false,true,false,false])
                            ,_U.list([false,false,false,false,false])]);
   var stillLife = _U.list([_U.list([false,false,false,false])
                           ,_U.list([false,true,true,false])
                           ,_U.list([false,true,true,false])
                           ,_U.list([false,false,false,false])]);
   var seedRow = F2(function (x,seed) {    var generator = A2($Random.list,x,$Random.bool);return A2($Random.generate,generator,seed);});
   var addRow = F4(function (n,x,list,seed) {
      addRow: while (true) {
         var _p0 = n;
         if (_p0 === 0) {
               return list;
            } else {
               var _p1 = A2(seedRow,x,seed);
               var row = _p1._0;
               var newSeed = _p1._1;
               var _v1 = n - 1,_v2 = x,_v3 = A2($List._op["::"],row,list),_v4 = newSeed;
               n = _v1;
               x = _v2;
               list = _v3;
               seed = _v4;
   ...
   var seedGrid = F3(function (x,y,seedInt) {    var initialSeed = $Random.initialSeed(seedInt);return A4(addRow,y,x,_U.list([]),initialSeed);});
   return _elm.SeedGenerator.values = {_op: _op,seedGrid: seedGrid};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $SeedGenerator = Elm.SeedGenerator.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Time = Elm.Time.make(_elm),
   $Window = Elm.Window.make(_elm);
   var _op = {};
   var step = F2(function (currentGrid,oldGrid) {    return $List.isEmpty(oldGrid) ? currentGrid : $Grid.nextGrid(oldGrid);});
   var getInitialSeed = F2(function (seed,windowDimension) {
      return A2($Signal.map,function (_p0) {    var _p1 = _p0;return A3($SeedGenerator.seedGrid,_p1._0,_p1._1,seed);},windowDimension);
   });
   var seed = 1989;
   var frequency = $Time.millisecond;
   var tickingGrid = function (grid) {    return A2($Signal.sampleOn,$Time.every(frequency),grid);};
   var cellSize = 30;
   var gridDimension = A2($Signal.map,
   function (_p2) {
      var _p3 = _p2;
      return {ctor: "_Tuple2",_0: _p3._0 / cellSize | 0,_1: _p3._1 / cellSize | 0};
   },
   $Window.dimensions);
   var main = function () {
      var initialSeed = A2(getInitialSeed,seed,gridDimension);
      var grid = A3($Signal.foldp,step,_U.list([]),tickingGrid(initialSeed));
      return A2($Signal.map,$Grid.drawGrid(cellSize),grid);
   }();
   return _elm.Main.values = {_op: _op
                             ...
                             ,getInitialSeed: getInitialSeed
                             ,step: step
                             ,tickingGrid: tickingGrid
                             ,main: main};
};
